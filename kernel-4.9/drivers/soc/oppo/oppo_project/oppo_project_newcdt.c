...
#define Ptr2UINT64(p)		(unsigned long long)(p)
...
static int __init update_feature(void)
{
	...
	set_fs(KERNEL_DS);
	if (oppo_info) {
               //#ifndef OPLUS_FEATURE_NFC
               //HuangRuhong@CONNECTIVITY.NFC.Basic.ALMId70839, 2020/05/01, Add for:P95Q support SN100 and NQ330 concurrently
               //if (get_oppo_feature(1) & 0x01) {
               //else
               if ((get_oppo_feature(1) & 0x01) || (get_project() == 19539)) {
               //#endif OPLUS_FEATURE_NFC
