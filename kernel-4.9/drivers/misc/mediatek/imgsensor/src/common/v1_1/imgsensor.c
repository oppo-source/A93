...
#ifdef VENDOR_EDIT
//WuSongbai@Cam.Drv, 2020/06/28, add for charge status change
...
MINT32 imgsensor_sensor_open(struct IMGSENSOR_SENSOR *psensor)
{
	...
	if (psensor_func && psensor_func->SensorOpen && psensor_inst) {
		...
		if (ret != ERROR_NONE) {
			...
			#ifdef VENDOR_EDIT
			//WuSongbai@Cam.Drv, 2020/06/28, add for charge status change
			if (is_project(OPPO_19536)||is_project(OPPO_19537)||is_project(OPPO_19538)||is_project(OPPO_19539)||is_project(OPPO_19541)){
...
static void imgsensor_init_sensor_list(void)
{
	...
	if (is_project(OPPO_19537) || is_project(OPPO_19538) || is_project(OPPO_19536) || is_project(OPPO_19539) || is_project(OPPO_19541)) {
...
int imgsensor_set_driver(struct IMGSENSOR_SENSOR *psensor)
{
	...
	/* Tan.Bowen@Camera.Driver 20200423 add for project 19537&19538*/
	} else if (is_project(OPPO_19537) || is_project(OPPO_19538) || is_project(OPPO_19536) || is_project(OPPO_19539) || is_project(OPPO_19541)) {
		imgsensor_i2c_init(&psensor_inst->i2c_cfg,
