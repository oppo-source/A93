...
int oppo_mt6360_pd_setup_forsvooc(void)
{
	...
	printk(KERN_ERR "%s: pd9v svooc [%d %d]", __func__, em_mode, chip->batt_volt);
	if (!chip->calling_on && !chip->camera_on && chip->charger_volt < 6500 && chip->soc < 90
		&& chip->temperature <= 530 && chip->cool_down_force_5v == false
		&& ((em_mode && chip->batt_volt < 4100) || (!em_mode && chip->batt_volt < 4000))) {
		if (pinfo->pd_type == MTK_PD_CONNECT_PE_READY_SNK_APDO) {
		...
		if (chip->charger_volt > 7500 &&
			...
			|| ((!em_mode && chip->batt_volt >= 4000) || (em_mode && chip->batt_volt >= 4100))	//en_mode
...
int oppo_chg_set_qc_config_forsvooc(void)
{
	...
	printk(KERN_ERR "%s: qc9v default [%d %d]", __func__, em_mode, chip->batt_volt);
	if (!chip->calling_on && !chip->camera_on && chip->charger_volt < 6500 && chip->soc < 90
		&& chip->temperature <= 530 && chip->cool_down_force_5v == false
		&& ((em_mode && chip->batt_volt < 4100) || (!em_mode && chip->batt_volt < 4000))) {	//
