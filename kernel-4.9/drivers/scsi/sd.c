...
#ifdef VENDOR_EDIT
/* Fuchun.Liao@BSP.Kernel.Stability 2020/08/27 add for scsi_suspend block issue ALPS05320942 */
		/**
		* when runtime suspending, double check if so many request in queue, 
		* queue is full and busy, can not get more request from queue, may hang.
		*/
		unsigned long flags = 0;
		unsigned int nr_congestion_off = 0;
		struct request_queue *q = sdp->request_queue;

		spin_lock_irqsave(q->queue_lock, flags);
		nr_congestion_off = q->nr_congestion_off;		// default 103
		if (nr_congestion_off < 90) {
			nr_congestion_off = 90;
		}
		if (q->rpm_status == RPM_SUSPENDING && 
			q->nr_pending > nr_congestion_off){
			spin_unlock_irqrestore(q->queue_lock, flags);
			if (retries == 3)
				return -EBUSY;
			else 
				break; /* next will check scsi_execute fail reason */
		}
		spin_unlock_irqrestore(q->queue_lock, flags);
#endif /* VENDOR_EDIT */
