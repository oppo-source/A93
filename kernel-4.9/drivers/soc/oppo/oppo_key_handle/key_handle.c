...
static oppo_key_handle_t password_match(void)
{
	int i;
...
void oppo_key_post_process(struct input_dev *dev, int key, int val) {
	int i;
	for(i = 0; i < OPPO_KEY_NUM; i++)
		if (OPPO_KEY_TASKS[i].open) (*OPPO_KEY_TASKS[i].post_process)(dev, key, val);
}

void oppo_key_process(struct input_dev *dev, int key, int val)
{
...
void oppo_key_event(struct input_dev *dev, int type, int key, int val)
{
...
void get_passwd_base(const char *val)
{
	u16 PASSWORD_BASE;
	int i;
	for(i = 0; i < OPPO_KEY_NUM; i++)
...
void register_oppo_key_task(oppo_key_handle_t type,
                                bool open,
                                u16 mask,
                                char* name,
                                void *process,
