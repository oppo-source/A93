...
static void read_eepromData(void)
{
	...
	for (idx = 0; idx <DUALCAM_CALI_DATA_LENGTH; idx++) {
		CAM_DUAL_DATA[idx] = read_cmos_eeprom_8(GC02M1B_STEREO_START_ADDR + idx);
	}
	for (idx = 0; idx <CAMERA_MODULE_SN_LENGTH; idx++) {
		CAM_SN[idx] = read_cmos_eeprom_8(0x40 + idx);
...
static kal_int32 write_Module_data(ACDK_SENSOR_ENGMODE_STEREO_STRUCT * pStereodata)
{
	...
	if(pStereodata != NULL) {
		...
		if ((pStereodata->uSensorId == GC02M1_SENSOR_ID0) && (data_length == DUALCAM_CALI_DATA_LENGTH)
...
static kal_uint32 get_imgsensor_id(UINT32 *sensor_id)
{
    ...
    LOG_INF("gc02m1_mipi_mono1_Sensor I2C_BUS = %d\n", I2C_BUS);
    if(I2C_BUS != 8){
