...
static void read_eepromData(void)
{
	...
	for (idx = 0; idx <DUALCAM_CALI_DATA_LENGTH; idx++) {
		CAM_DUAL_DATA[idx] = read_cmos_eeprom_8(HI846_STEREO_START_ADDR_19537 + idx);
	}
...
static kal_int32 write_Module_data(ACDK_SENSOR_ENGMODE_STEREO_STRUCT * pStereodata)
{
	...
	if(pStereodata != NULL) {
		...
		if ((pStereodata->uSensorId == HI846_SENSOR_ID) && (data_length == DUALCAM_CALI_DATA_LENGTH)
			...
			write_eeprom_protect(1);
			msleep(6);
			LOG_INF("com_0:0x%x\n", read_cmos_eeprom_8(HI846_STEREO_START_ADDR_19537));
			LOG_INF("com_39:0x%x\n", read_cmos_eeprom_8(HI846_STEREO_START_ADDR_19537+39));
			LOG_INF("innal_40:0x%x\n", read_cmos_eeprom_8(HI846_STEREO_START_ADDR_19537+40));
			LOG_INF("innal_1556:0x%x\n", read_cmos_eeprom_8(HI846_STEREO_START_ADDR_19537+1556));
			LOG_INF("tail1_1557:0x%x\n", read_cmos_eeprom_8(HI846_STEREO_START_ADDR_19537+1557));
			LOG_INF("tail2_1558:0x%x\n", read_cmos_eeprom_8(HI846_STEREO_START_ADDR_19537+1558));
			LOG_INF("tail3_1559:0x%x\n", read_cmos_eeprom_8(HI846_STEREO_START_ADDR_19537+1559));
			LOG_INF("tail4_1560:0x%x\n", read_cmos_eeprom_8(HI846_STEREO_START_ADDR_19537+1560));
