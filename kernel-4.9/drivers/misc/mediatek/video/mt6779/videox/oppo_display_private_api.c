...
bool oppo_display_meta_idle_support;
...
int lcm_first_get_serial(void)
{
	...
	if(lcm_param->panel_serial_num == 0)
		lcm_param->panel_serial_num = PANEL_SERIAL_NUM_REG;

	printk("lcm_param->panel_serial_num is 0x:%x",lcm_param->panel_serial_num);

	if (oppo_display_panelnum_support) {
		pr_err("lcm_first_get_serial\n");
		ret = panel_serial_number_read(lcm_param->panel_serial_num, &serial_number,
				PANEL_REG_READ_LEN);
	}
	return ret;
}
				...
				struct device_attribute *attr, char *buf)
{
	...
	if (oppo_display_panelnum_support) {
		if (serial_number == 0) {
			ret = primary_display_read_serial(lcm_param->panel_serial_num, &serial_number,
				PANEL_REG_READ_LEN);
		}
