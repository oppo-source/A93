...
int oppo_OTA_upgrade = 0;
unsigned int last_power =0;
...
#ifdef VENDOR_EDIT
void set_upgrade_flag(int upgrade_flag)
{
	static unsigned int last_oppo_OTA_upgrade;
	last_oppo_OTA_upgrade = oppo_OTA_upgrade;
	oppo_OTA_upgrade = upgrade_flag;

	if (last_oppo_OTA_upgrade && !upgrade_flag) {
		mt_ppm_cpu_thermal_protect(last_power);
	}
	ppm_err("set_upgrade_flag:%d\n",oppo_OTA_upgrade);
...
#ifdef VENDOR_EDIT
// ziyu.qi@OTA upgrade, 2020/08/16, change OTA thermal control
	if (oppo_OTA_upgrade) {
		last_power = limited_power;
		limited_power += 100;
	}
#endif
	...
	ppm_err("Get budget from thermal => limited_power = %d--%d\n",
		limited_power,oppo_OTA_upgrade);
