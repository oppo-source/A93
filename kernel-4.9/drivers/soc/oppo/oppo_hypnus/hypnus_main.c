...
long hypnus_ioctl_set_upgrade_flag(struct hypnus_data *pdata,
	unsigned int cmd, void *data)
{
	struct hypnus_upgrade_prop *prop = data;

	if (!pdata->cops->set_upgrade_flag)
		return -ENOTSUPP;

	return pdata->cops->set_upgrade_flag(prop->upgrade_flag);
}
...
long hypnus_ioctl_set_anim_boost(struct hypnus_data *pdata,
	unsigned int cmd, void *data)
{
	struct hypnus_anim_boost_prop *prop = data;

	if (!pdata->cops->set_anim_boost)
		return -ENOTSUPP;

	return pdata->cops->set_anim_boost(prop->anim_boost);
}
