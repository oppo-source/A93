...
ssize_t get_oppo_rpm_stats(char *ToUserBuf,
			  size_t sz, void *priv)
{
	char *p = ToUserBuf;
#undef log
...
EXPORT_SYMBOL(get_oppo_rpm_stats);

ssize_t get_oppo_rpm_master_stats(char *ToUserBuf,
			  size_t sz, void *priv)
{
		/* dump subsystem sleep info */
#if defined(CONFIG_MTK_ECCCI_DRIVER)
...
EXPORT_SYMBOL(get_oppo_rpm_master_stats);
