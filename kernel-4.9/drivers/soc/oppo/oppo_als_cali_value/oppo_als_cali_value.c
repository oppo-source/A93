...
#define SENSOR_ALSPARA_DELAY_FIRST_TIME  5000
...
static void sensor_para_work(struct work_struct *work)
{
    ...
    if ((prj == 19536) || (prj == 19537) || (prj == 19538) || (prj == 19539) || (prj == 19541)) {
        while (count <= 3) {
            printk("oppo_als_cali_para get_sensor_name = %d",get_sensor_name(ID_LIGHT));
            if (get_sensor_name(ID_LIGHT) == ALSPS_OPPOPLAT_TCS3701) {
              gdata->row_coe = 110;
              break;
            }
            else if (get_sensor_name(ID_LIGHT) == ALSPS_OPPOPLAT_STK32600) {
              gdata->row_coe = 65;
              break;
            }
