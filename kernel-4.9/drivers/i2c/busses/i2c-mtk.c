...
#define I2C_STATE_SCL "i2c-state_scl"
#define OUTPUT_LOW_STATE_SCL "output-low-state_scl"
static int fg_device_type = 0;
static bool is_fg_iic_error = false;
static bool i2c_err_occured = false;
static bool is_charger_exit = false;
...
bool oplus_is_fg_iic_error(bool is_iic_error)
{
	if (is_iic_error) {
		is_fg_iic_error = true;
		return true;
	} else if (is_fg_iic_error) {
		is_fg_iic_error = false;
		return true;
	}
	return false;
}
EXPORT_SYMBOL(oplus_is_fg_iic_error);
void oplus_is_chargering(bool is_chargering)
{
	if (is_chargering) {
		is_charger_exit = true;
	} else {
		is_charger_exit = false;
	}
}
EXPORT_SYMBOL(oplus_is_chargering);
...
/*zhijie.Li@BSP.CHG.Basic, 2019/11/26, lzj Modify for I2C5(high speed mode) duty ratio*/
	...
	if (!strcmp(dev_name(i2c->dev), chg_i2c) && i2c->addr == FG_DEVICE_ADDR) {
		if (oplus_is_fg_iic_error(false)) {
			tmo = 0;
			iic_need_reset = true;
		}
...
/*Jianchao.Shi@PSW.BSP.CHG.Basic, 2019/07/01, sjc Add for zhongying fg ZY0602*/
		if (!strcmp(dev_name(i2c->dev), chg_i2c) && i2c->addr == FG_DEVICE_ADDR) {
			dev_err(i2c->dev, "[OPPO_TEST] %s, %x %d\n", dev_name(i2c->dev), i2c->addr, err_count_for_reset);
			if (oppo_get_fg_device_type() == DEVICE_TYPE_ZY0602) {
				if ((err_count_for_reset >= 5 || iic_need_reset) && is_charger_exit == false) {
					i2c_gpio_reset(i2c);
