...
extern int wakeup_softirqd_when_bootup(void);
#endif /* VENDOR_EDIT */
...
/* Fuchun.Liao@BSP.Kernel.Stability 2020/07/16 modify for block_softirq pending issue */
	if (iowait_count && max_wakeup_count > 0) {
		if (wakeup_softirqd_when_bootup()) {
			max_wakeup_count--;
			pr_info("wakeup_softirqd_when_bootup count = %d iowait_count = %d\n",
				max_wakeup_count, iowait_count);
		}
	}
