...
#ifdef VENDOR_EDIT
//zhouhengguo@BSP.KERNEL.STABILITY, 2020.6.3, avoid lose sdcard data within shutdown sync
extern bool is_sync_ms_fs;
#endif /*VENDOR_EDIT*/
...
static void hang_dump_backtrace(void)
{
	...
	for_each_process(p) {
		get_task_struct(p);
#ifdef VENDOR_EDIT
//zhouhengguo@BSP.KERNEL.STABLITY, 2020.4.13, add memory rss data for hang problem
		if (p->mm != NULL)
			Log2HangInfo("Pid=%d, Name=%s, RSS=%luKB\n", p->pid, p->comm, get_mm_rss(p->mm) << (PAGE_SHIFT - 10));
#endif /*VENDOR_EDIT*/
...
#ifdef VENDOR_EDIT
/* Fuchun.Liao@BSP.Kernel.Stability 2020/07/16 modify for block_softirq pending issue */
struct wakeup_softirqd_data {
	struct workqueue_struct *softirqd_wq0;
	struct workqueue_struct *softirqd_wq1;
	struct workqueue_struct *softirqd_wq2;
	struct workqueue_struct *softirqd_wq3;
	struct workqueue_struct *softirqd_wq4;
	struct workqueue_struct *softirqd_wq5;
	struct workqueue_struct *softirqd_wq6;
	struct workqueue_struct *softirqd_wq7;
	
	struct delayed_work softirqd_work0;
	struct delayed_work softirqd_work1;
	struct delayed_work softirqd_work2;
	struct delayed_work softirqd_work3;
	struct delayed_work softirqd_work4;
	struct delayed_work softirqd_work5;
	struct delayed_work softirqd_work6;
	struct delayed_work softirqd_work7;
	
	int wq_init;
};

static struct wakeup_softirqd_data *wakeup_softirqd_info = NULL;

extern void wakeup_softirqd_with_log(void);
extern void show_softirq_pending(void);
extern void show_ipi_history(void);
extern void check_scsi_device_cmdlist(void);
extern void monitor_blk_softirq_init(void);

static void wakeup_sofirqd_work(struct work_struct *work)
{
	wakeup_softirqd_with_log();
}
...
#define CPU7			7
#define ALL_CPU		8
void wakeup_softirqd_oncpu(int cpu_num)
{
	if (!wakeup_softirqd_info) {
		return;
	}
	if (!wakeup_softirqd_info->wq_init) {
		return;
	}
	pr_info("%s cpu_num: %d call\n", __func__, cpu_num);
	switch(cpu_num) {
	case ALL_CPU:
		queue_delayed_work_on(6, wakeup_softirqd_info->softirqd_wq6, 
			&wakeup_softirqd_info->softirqd_work6, 0);
		msleep(200);
		queue_delayed_work_on(7, wakeup_softirqd_info->softirqd_wq7, 
			&wakeup_softirqd_info->softirqd_work7, 0);
		msleep(200);
		queue_delayed_work_on(0, wakeup_softirqd_info->softirqd_wq0, 
			&wakeup_softirqd_info->softirqd_work0, 0);
		msleep(200);
		queue_delayed_work_on(1, wakeup_softirqd_info->softirqd_wq1, 
			&wakeup_softirqd_info->softirqd_work1, 0);
		msleep(200);
		queue_delayed_work_on(2, wakeup_softirqd_info->softirqd_wq2, 
			&wakeup_softirqd_info->softirqd_work2, 0);
		msleep(200);
		queue_delayed_work_on(3, wakeup_softirqd_info->softirqd_wq3, 
			&wakeup_softirqd_info->softirqd_work3, 0);
		msleep(200);
		queue_delayed_work_on(4, wakeup_softirqd_info->softirqd_wq4, 
			&wakeup_softirqd_info->softirqd_work4, 0);
		msleep(200);
		queue_delayed_work_on(5, wakeup_softirqd_info->softirqd_wq5, 
			&wakeup_softirqd_info->softirqd_work5, 0);
		break;
...
static int wakeup_softirqd_info_init(void)
{
	if (wakeup_softirqd_info) {
		//pr_info("%s have inited\n", __func__);
		return 0;
	}
	wakeup_softirqd_info = kzalloc(sizeof(struct wakeup_softirqd_data), GFP_KERNEL);
	if (!wakeup_softirqd_info) {
		pr_info("%s malloc fail\n", __func__);
		goto err;
	}
	wakeup_softirqd_info->softirqd_wq0 = alloc_workqueue("wakeup_softirqd0", WQ_MEM_RECLAIM, 0);
	wakeup_softirqd_info->softirqd_wq1 = alloc_workqueue("wakeup_softirqd1", WQ_MEM_RECLAIM, 0);
	wakeup_softirqd_info->softirqd_wq2 = alloc_workqueue("wakeup_softirqd2", WQ_MEM_RECLAIM, 0);
	wakeup_softirqd_info->softirqd_wq3 = alloc_workqueue("wakeup_softirqd3", WQ_MEM_RECLAIM, 0);
	wakeup_softirqd_info->softirqd_wq4 = alloc_workqueue("wakeup_softirqd4", WQ_MEM_RECLAIM, 0);
	wakeup_softirqd_info->softirqd_wq5 = alloc_workqueue("wakeup_softirqd5", WQ_MEM_RECLAIM, 0);
	wakeup_softirqd_info->softirqd_wq6 = alloc_workqueue("wakeup_softirqd6", WQ_MEM_RECLAIM, 0);
	wakeup_softirqd_info->softirqd_wq7 = alloc_workqueue("wakeup_softirqd7", WQ_MEM_RECLAIM, 0);
	if (wakeup_softirqd_info->softirqd_wq0 == NULL
		||wakeup_softirqd_info->softirqd_wq1 == NULL
		|| wakeup_softirqd_info->softirqd_wq2 == NULL
		|| wakeup_softirqd_info->softirqd_wq3 == NULL
		|| wakeup_softirqd_info->softirqd_wq4 == NULL
		|| wakeup_softirqd_info->softirqd_wq5 == NULL
		|| wakeup_softirqd_info->softirqd_wq6 == NULL
		|| wakeup_softirqd_info->softirqd_wq7 == NULL) {
		goto err;
	}
	INIT_DELAYED_WORK(&wakeup_softirqd_info->softirqd_work0, wakeup_sofirqd_work);
	INIT_DELAYED_WORK(&wakeup_softirqd_info->softirqd_work1, wakeup_sofirqd_work);
	INIT_DELAYED_WORK(&wakeup_softirqd_info->softirqd_work2, wakeup_sofirqd_work);
	INIT_DELAYED_WORK(&wakeup_softirqd_info->softirqd_work3, wakeup_sofirqd_work);
	INIT_DELAYED_WORK(&wakeup_softirqd_info->softirqd_work4, wakeup_sofirqd_work);
	INIT_DELAYED_WORK(&wakeup_softirqd_info->softirqd_work5, wakeup_sofirqd_work);
	INIT_DELAYED_WORK(&wakeup_softirqd_info->softirqd_work6, wakeup_sofirqd_work);
	INIT_DELAYED_WORK(&wakeup_softirqd_info->softirqd_work7, wakeup_sofirqd_work);
	
	wakeup_softirqd_info->wq_init = 1;
	pr_info("%s success\n", __func__);
	return 0;
err:
	wakeup_softirqd_info = NULL;
	pr_info("%s failed\n", __func__);
	return -1;
}
...
/* Fuchun.Liao@BSP.Kernel.Stability 2020/07/16 modify for block_softirq pending issue */
int wakeup_softirqd_when_bootup(void)
{
	return 0;		//disable it
	if ((hd_timeout != 0x7fffffff)) {
		return 0;
	}
	wakeup_softirqd_oncpu(ALL_CPU);
	return 1;
}
#endif
...
#endif
#ifdef VENDOR_EDIT
//zhouhengguo@BSP.KERNEL.STABLITY, 2020.4.26, send uevent to usersapce for hang 150s
			if (hd_timeout == 10 && hang_detect_counter == 6) {
				char *envp_ext[2] = {"HANG_DETECT10_NOTIFY", NULL};
				pr_info("[Hang_Detect] send uevent to userspace for counts down %d:%d\n", hang_detect_counter, hd_timeout);
				kobject_uevent_env(&aed_wdt_RT_Monitor_dev.this_device->kobj, KOBJ_CHANGE, envp_ext);
			}
#endif /*VENDOR_EDIT*/

#ifdef VENDOR_EDIT
//zhouhengguo@BSP.KERNEL.STABILITY, 2020.6.3, avoid lose sdcard data within shutdown sync
			if (hd_timeout == 3 && hang_detect_counter == -1 && is_sync_ms_fs) {
				pr_info("[Hang_Detect] shutdown sdcard sync timeout for counts down %d:%d\n", hang_detect_counter, hd_timeout);
				pr_info("[Hang_Detect] avoid to lose customs sdcard data, add 120s time\n");
				hd_timeout =4;
				hang_detect_counter = 4;
			}
#endif /*VENDOR_EDIT*/

#ifdef VENDOR_EDIT
/* Fuchun.Liao@BSP.Kernel.Stability 2020/07/16 modify for block_softirq pending issue */
			if (((hd_timeout == 12) || (hd_timeout == 14)) && 
				(hang_detect_counter == 14 || hang_detect_counter == 12
					|| hang_detect_counter == 11 || hang_detect_counter == 9)) {
				pr_info("[Hang_Detect] wakeup softirqd all for counts down %d:%d\n", hang_detect_counter, hd_timeout);
				show_ipi_history();
				show_softirq_pending();
				check_scsi_device_cmdlist();
				wakeup_softirqd_oncpu(ALL_CPU);
			}
#endif /* VENDOR_EDIT */
...
#ifdef VENDOR_EDIT
/* Fuchun.Liao@BSP.Kernel.Stability 2020/07/16 modify for block_softirq pending issue */
	wakeup_softirqd_info_init();
	monitor_blk_softirq_init();
#endif	
	return 0;
}
