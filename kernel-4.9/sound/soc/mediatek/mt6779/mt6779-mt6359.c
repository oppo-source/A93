...
#ifdef VENDOR_EDIT
/* Zhao.Pan@MULTIMEDIA.AUDIODRIVER.SMARTPA, 2020/04/23,
 * add for PA aw87339  */
...
static int use_aw87339 = 0;
static int set_FM_CE = 0;
enum oppo_pa_type {
	OPPO_PA_NXP = 0,
	OPPO_PA_AWINIC,
	OPPO_PA_TYPE_NUM
...
enum oppo_FM_CE_Satisfy{
	OPPO_FM_CE_ENABLE,
	OPPO_FM_CE_DISABLE
};

static const char *const mt6779_oppo_FM_CE_Satisfy[] = {"OPPO_FM_CE_ENABLE",
						      "OPPO_FM_CE_DISABLE"};
static const char *const mt6779_oppo_pa_type[] = {"OPPO_PA_NXP",
						      "OPPO_PA_AWINIC"};
#endif /*VENDOR_EDIT*/
...
static const struct soc_enum mt6779_spk_type_enum[] = {
	...
	SOC_ENUM_SINGLE_EXT(ARRAY_SIZE(mt6779_oppo_pa_type),
			    mt6779_oppo_pa_type),
	/* Kun.Zhao@MM.AudioDriver.Machine, 2020/05/21, add for CE satisfy */
	SOC_ENUM_SINGLE_EXT(ARRAY_SIZE(mt6779_oppo_FM_CE_Satisfy),
			    mt6779_oppo_FM_CE_Satisfy),
	#endif /* VENDOR_EDIT */
};
...
/* Kun.Zhao@MM.AudioDriver.Machine, 2020/05/21, add for CE satisfy */
static int mt6779_FM_CE_Satisfy_get(struct snd_kcontrol *kcontrol,
			       struct snd_ctl_elem_value *ucontrol)
{
	int idx = set_FM_CE ? OPPO_FM_CE_ENABLE: OPPO_FM_CE_DISABLE;
	pr_info("%s() = %d\n", __func__, idx);
	ucontrol->value.integer.value[0] = idx;
	return 0;
}

static int mt6779_pa_type_get(struct snd_kcontrol *kcontrol,
			       struct snd_ctl_elem_value *ucontrol)
{
	int idx = use_aw87339 ? OPPO_PA_AWINIC : OPPO_PA_NXP;
...
static const struct snd_kcontrol_new mt6779_mt6359_controls[] = {
	...
	SOC_ENUM_EXT("OPPO_PA_TYPE_GET", mt6779_spk_type_enum[3],
		     mt6779_pa_type_get, NULL),
	/* Kun.Zhao@MM.AudioDriver.Machine, 2020/05/21, add for CE satisfy */
	SOC_ENUM_EXT("OPPO_FM_CE_SATISFY", mt6779_spk_type_enum[4],
		     mt6779_FM_CE_Satisfy_get, NULL),
	#endif /* VENDOR_EDIT */
};
...
#else
	ret = of_property_read_u32(pdev->dev.of_node, "use_awinic_normal_pa",
		&use_aw87339);
	if (!ret) {
		dev_info(&pdev->dev, "get use_awinic_normal_pa = %d\n", use_aw87339);
	}
	/* Kun.Zhao@MM.AudioDriver.Machine, 2020/05/21, add for CE satisfy */
	ret = of_property_read_u32(pdev->dev.of_node, "use_FM_CE",
		&set_FM_CE);
	if (!ret) {
		dev_info(&pdev->dev, "get use_satisfy_CE = %d\n", set_FM_CE);
	}
	/* Yongzhi.Zhang@PSW.MM.AudioDriver.Machine, 2018/11/06,
