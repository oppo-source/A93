...
static int subboard_verify(struct devinfo_data *const devinfo_data)
{
	...
	if(devinfo_data->sub_hw_id2 >= 0 ) {
		DEVINFO_ERR("id2(%d) = %d Operator = %d", devinfo_data->sub_hw_id2, id2, operator);
	}
	...
	switch(get_project()) {
		...
		case OPPO_19536:
		case OPPO_19541:
			//in 6779Q, id value: 0->pull down;1->high resistance;2->pull up, this is different from hardware talbe for P95, so this value is different form hardware table for compatible
			if(id1 == 1) {
				snprintf(devinfo_data->sub_mainboard_info.manufacture, INFO_BUF_LEN, "rf-match");
			}else {
				snprintf(devinfo_data->sub_mainboard_info.manufacture, INFO_BUF_LEN, "rf-unmatch");
