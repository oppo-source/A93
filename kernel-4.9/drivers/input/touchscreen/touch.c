...
int tp_util_get_vendor(struct hw_resource *hw_res, struct panel_info *panel_data)
{
    ...
    case 19539:
    case 19541:
             panel_data->test_limit_name = kzalloc(sizeof(GT9886_BASELINE_TEST_LIMIT_NAME_19537), GFP_KERNEL);
             if (panel_data->test_limit_name == NULL) {
                 pr_err("[TP]panel_data.test_limit_name kzalloc error\n");
                 return -1;
             }
             strcpy(panel_data->test_limit_name, GT9886_BASELINE_TEST_LIMIT_NAME_19537);
             strcpy(panel_data->fw_name, GT9886_FW_NAME_19537);
			 memcpy(panel_data->manufacture_info.version, "0xdd3400000", 11);
             pr_err("[TP]%s: fw_name = %s \n",__func__, panel_data->fw_name);
			break;
	}
