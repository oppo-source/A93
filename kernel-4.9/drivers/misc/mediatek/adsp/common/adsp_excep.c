...
void adsp_aed(enum adsp_excep_id type, enum adsp_core_id id)
{
	...
	n += snprintf(detail + n, ADSP_AED_STR_LEN - n, "%s",
		      coredump->assert_log);
#ifdef VENDOR_EDIT
	/* Liang.Wang@PSW.MM.AudioDriver.ADSP, 2020/05/18, modify log level for adsp exception */
	pr_info("%s", detail);
#else
	pr_debug("%s", detail);
#endif
...
#ifdef VENDOR_EDIT
	/* Zhao.Pan@MULTIMEDIA.AUDIODRIVER.FEATURE.SSR, 2020/06/17,add for adsp ramdump feedback*/
	scnprintf(fb_str, sizeof(fb_str), "NULL$$EventID@@%d$$EventData@@%d$$PackageName@@%s:task:%s,%s",
			OPPO_MM_AUDIO_EVENT_ID_ADSP_RESET, id, aed_type, coredump->task_name, coredump->assert_log);
