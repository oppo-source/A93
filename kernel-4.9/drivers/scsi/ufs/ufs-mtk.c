...
#ifdef VENDOR_EDIT
/* Fuchun.Liao@BSP.Kernel.Stability 2020/08/21 add for io block issue debug */
#define DATA_SYSTEM_LUN			2
static struct scsi_device *get_scsi_device(u64 lun)
{
	struct scsi_device *sdev = NULL;
	struct Scsi_Host *shost = NULL;

	if (!ufs_mtk_hba || !ufs_mtk_hba->host) {
		return NULL;
	}

	shost = ufs_mtk_hba->host;
...
void check_scsi_device_cmdlist(void)
{
	struct scsi_device *sdev = NULL;

	sdev = get_scsi_device(DATA_SYSTEM_LUN);
	if (!sdev || !sdev->request_queue) {
		return;
	}
	if (!sdev->request_queue) {
		pr_info("%s request_queue  NULL\n", __func__);
		return;
	} 
	
	if (list_empty(&sdev->request_queue->queue_head)) {
		pr_info("%s queue_head NULL\n", __func__);
	} else {
		pr_info("%s queue_head not NULL\n", __func__);
	}

	if (list_empty(&sdev->cmd_list)) {
		pr_info("%s cmd_list NULL\n", __func__);
	} else {
		pr_info("%s cmd_list not NULL\n", __func__);
	}
}
