...
#ifdef VENDOR_EDIT
// ziyu.qi@cpu_cooling, 2020/08/12, add for ux boost
static int oppo_userlimit_cpu_freq(int boost)
{
	struct ppm_limit_data freq_limit[2];
	struct ppm_limit_data NO_limit[2];

	freq_limit[0].max = 3000000;
	freq_limit[0].min = -1;
	freq_limit[1].max = 3000000;
	freq_limit[1].min = -1;
	...
	if (boost){
		/******request max cpu freq*********/
		mt_ppm_hard_userlimit_cpu_freq(2, freq_limit);

	} else {
		/******release request*********/
		mt_ppm_hard_userlimit_cpu_freq(2, NO_limit);
	}

	return 0;
}

void set_anim_boost(int boost)
{
	oppo_userlimit_cpu_freq(boost);
