...
TRACE_EVENT(block_time,
	...
	TP_fast_assign(
		...
		blk_fill_rwbs(__entry->rwbs, req_op(rq), rq->cmd_flags, blk_rq_bytes(rq));

		blk_dump_cmd(__get_str(cmd), rq);
	),

	TP_printk("%d,%d,%d,%llu,%llu,%d,%llu,%s,(%s),%llu,%u,%u,[%d]",
		  MAJOR(__entry->dev), MINOR(__entry->dev),
		  __entry->current_uid,__entry->flash_io_latency ,
		  __entry->delta_us,__entry->tag,
		  __entry->now,__entry->rwbs, __get_str(cmd),
		  (unsigned long long)__entry->sector,
		  __entry->nr_sector,__entry->bytes, __entry->errors)
);
