...
void dump_process(struct input_dev *dev, int key, int val)
{
...
void dump_post_process(struct input_dev *dev, int key, int val)
{
...
void dump_close_process(void) {
	if (OPPO_KEY_TASKS[OPPO_KEY_DUMP].open == 0) {
		oppo_switch_fulldump(false);
	}
}

void OPPO_DUMP(const char f[24])
{
	if (OPPO_KEY_TASKS[OPPO_KEY_DUMP].open) {
		pr_err("%s: enter dump\n", f);
		BUG_ON(1);
